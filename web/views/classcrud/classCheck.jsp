<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="classs.model.vo.Classs,java.util.ArrayList"%>
	...
	<div class="w3-container w3-content"
		...
		<div class="w3-row">
			...
			<div id="pagebox" align="center">
				<a href="/eunsu/classcheck?page=1&pno=<%=loginProfessor.getProfessorNo() %>">|◁</a> &nbsp;
				<%
					if ((beginPage - 10) < 1) {
				%>
				<a href="/eunsu/classcheck?page=1&pno=<%=loginProfessor.getProfessorNo() %>">◀◀</a>
				...
				%>
				<a href="/eunsu/classcheck?page=<%=beginPage - 10%>&pno=<%=loginProfessor.getProfessorNo() %>">◀◀</a>
				<%
					}
				%>
				&nbsp;
				<%
					for (int p = beginPage; p <= endPage; p++) {
						if (p == currentPage) {
				%>
				<a href="/eunsu/classcheck?page=<%=p%>&pno=<%=loginProfessor.getProfessorNo() %>"><font
					color="red"><b>[<%=p%>]
					</b></font></a>
				<%
					} else {
				%>
				<a href="/eunsu/classcheck?page=<%=p%>&pno=<%=loginProfessor.getProfessorNo() %>"><%=p%></a>
				<%
					}
					}
				%>
				...
				%>
				<a href="/eunsu/classcheck?page=<%=maxPage%>&pno=<%=loginProfessor.getProfessorNo() %>">▶▶</a>
				<%
					} else {
				%>
				<a href="/eunsu/classcheck?page=<%=endPage + 10%>&pno=<%=loginProfessor.getProfessorNo() %>">▶▶</a>
				<%
					}
				%>
				&nbsp; <a href="/eunsu/classcheck?page=<%=maxPage%>&pno=<%=loginProfessor.getProfessorNo() %>">▷|</a>
