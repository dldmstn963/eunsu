...
<body class="w3-theme-l5">
	...
	<div class="w3-container w3-content"
		...
		<div class="w3-row">
			...
			<!-- End Grid -->
		사용할 아이디 : <input type="text" id="sender" size="10" value="<%=loginEmployee.getEmployeeNo()%>"readonly> <br>
...
<div style="display:none;" id="chatbox">
	<fieldset>
		...
		<input type="text" id="inputMessage" name="inputMessage" onkeyup="enterKey();">
		<input type="button" value="보내기" onclick="send();" id="send" >
	...
	*/
	...
	function send(){
		...
		if($inputMessage.val() == "") {
			...
			$.ajax({
				url : "/eunsu/chatting",
				data : {inputMessage : $inputMessage.val(),
						sender:$('#sender').val(),
			...
			$textarea.html($textarea.html() + 
					"<p>"+d.getFullYear()+"."+(d.getMonth()+1)+"."+d.getDate()+" "+d.getHours()+":"+d.getMinutes()+"</p><p class='chat_content'>나 : "
				+ $inputMessage.val() + "</p>");
			webSocket.send($('#sender').val() + "|"
				+ $inputMessage.val());
	...
	function onMessage(event){
		...
		if(content == "" || 
			...
			$textarea.html($textarea.html() +
					...
					+ receiverID + " : " + content 
					+ "</p><br><br><p>"+d.getFullYear()+"."+(d.getMonth()+1)+"."+d.getDate()+" "+d.getHours()+":"+d.getMinutes()+"</p>");
			//화면이 위로 스크롤되게 처리함
			objDiv.scrollTop = objDiv.scrollHeight;
