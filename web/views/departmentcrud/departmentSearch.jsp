<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@ page import="employee.model.vo.Employee, department.model.vo.Department, java.util.ArrayList"%>
<%
	...
	String searchopen = ((String)request.getAttribute("searchopen").toString());
%>
<!DOCTYPE html>
<html>
<title>수은 대학교</title>
...
$(function (){
	...
	$("#departNameSortD").click(function(){
		sort = 2;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#departNameSortA").click(function(){
		sort = 3;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#deprtCategorySortD").click(function(){
		sort = 4;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#deprtCategorySortA").click(function(){
		sort = 5;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#departOpenSortD").click(function(){
		sort = 6;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#departOpenSortA").click(function(){
		sort = 7;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#departCapacitySortD").click(function(){
		sort = 8;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
	...
	$("#departCapacitySortA").click(function(){
		sort = 9;
		$.ajax({
			url:"/eunsu/departmentsearch",
			data : {sort : sort},
...
<body class="w3-theme-l5">
	...
	<div class="w3-container w3-content"
		...
		<form action="/eunsu/departmentsearch" align="center">
			학과 번호 : <input type="text" style="width: 90px;" name="searchno" value="">
			&nbsp;&nbsp;
			학과 명 : <input type="text" style="width: 170px;"name="searchname" value=""><br><br>
			학과 분류 : <select name="searchtype">
		...
		<form action="/eunsu/departmentupdate">
		<table align="center" border="1" cellspacing="0" cellpadding="3">
			<tr>
			<th>체크 박스</th>
			<th>학과 번호<button id="departNoSortD">↓</button><button id="departNoSortA">↑</button></th>
			<th>학과 이름<button id="departNameSortD">↓</button><button id="departNameSortA">↑</button></th>
			<th>학과 분류<button id="deprtCategorySortD">↓</button><button id="deprtCategorySortA">↑</button></th>
			<th>오픈 여부<button id="departOpenSortD">↓</button><button id="departOpenSortA">↑</button></th>
			<th>정원<button id="departCapacitySortD">↓</button><button id="departCapacitySortA">↑</button></th>
			</tr>
			<% for(Department d : list){ %>	
			<tr>
			<td align="center"><input type="checkbox" id="checkbox" name="checkbox" value="<%=d.getDepartmentNo()%>"></td>
			<td style="display:none;"><input type="text" name="departNo" value="<%= d.getDepartmentNo()%>"></td>
			<td><%= d.getDepartmentNo() %></td>
			<td><input type="text" style="width: 170px;" value="<%= d.getDepartmentName()%>" name="departName"></td>
			<td><input type="text" style="width: 70px;" value="<%= d.getCategory()%>" name="deprtCategory"></td>
			<td><input type="text" style="width: 20px;" value="<%= d.getOpenYN()%>" name="departOpen"></td>
			<td><input type="number" style="width: 55px;" value="<%= d.getCapacity()%>" name="departCapacity"></td>
			</tr>
			<%} %>	
		</table>
		<br>
		...
		<div id="pagebox" align="center">
			<a href="/eunsu/departmentsearch?page=1&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>">|◁</a>&nbsp;
			<% if((beginPage - 10) < 1){ %>
			<a href="/eunsu/departmentsearch?page=1&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>">◀◀</a>&nbsp;
			<%}else{ %>
			<a href="/eunsu/departmentsearch?page=<%= beginPage - 10%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>">◀◀</a>&nbsp;
			<%} %>
			<% for(int p = beginPage; p <= endPage; p++){
				if(p == currentPage){%>
				<a href="/eunsu/departmentsearch?page=<%= p%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>"><font color="red"><b>[<%=p %>]</b></font></a>&nbsp;			
				<%}else{ %>
				<a href="/eunsu/departmentsearch?page=<%= p%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>"><%=p %></a>&nbsp;
				<%}} %>
			
			<% if((endPage + 10)>maxPage){%>
				<a href="/eunsu/departmentsearch?page=<%= maxPage%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>">▶▶</a>&nbsp;
			<%}else{ %>
				<a href="/eunsu/departmentsearch?page=<%= endPage + 10%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>">▶▶</a>&nbsp;
			<%} %>
				<a href="/eunsu/departmentsearch?page=<%= maxPage%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchtype=<%=searchtype %>&searchopen=<%= searchopen%>">▷|</a>&nbsp;
