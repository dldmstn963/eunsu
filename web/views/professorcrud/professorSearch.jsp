...
<body class="w3-theme-l5">
	...
	<div class="w3-container w3-content"
		...
		<div id="pagebox" align="center">
			<a href="/eunsu/professorsearch?page=1&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>">|◁</a>&nbsp;
			<% if((beginPage - 10) < 1){ %>
			<a href="/eunsu/professorsearch?page=1&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>">◀◀</a>&nbsp;
			<%}else{ %>
			<a href="/eunsu/professorsearch?page=<%= beginPage - 10%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>">◀◀</a>&nbsp;
			<%} %>
			<% for(int p = beginPage; p <= endPage; p++){
				if(p == currentPage){%>
				<a href="/eunsu/professorsearch?page=<%= p%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>"><font color="red"><b>[<%=p %>]</b></font></a>&nbsp;			
				<%}else{ %>
				<a href="/eunsu/professorsearch?page=<%= p%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>"><%=p %></a>&nbsp;
				<%}} %>
			
			<% if((endPage + 10)>maxPage){%>
				<a href="/eunsu/professorsearch?page=<%= maxPage%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>">▶▶</a>&nbsp;
			<%}else{ %>
				<a href="/eunsu/professorsearch?page=<%= endPage + 10%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>">▶▶</a>&nbsp;
			<%} %>
				<a href="/eunsu/professorsearch?page=<%= maxPage%>&searchno=<%= searchno%>&searchname=<%=searchname %>&searchdepart=<%=searchdepart %>">▷|</a>&nbsp;
